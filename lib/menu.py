...
from lib.graphics import Spritesheet
...
class Menu:
    def __init__(self, screen, dashboard, level, sound):
        ...
        self.levelCount = 0
        self.spritesheet = Spritesheet("./assets/images/title_screen.png")
    ...
    def saveSettings(self, url):
        data = {"sound": self.music, "sound": self.sfx}
    ...
    def drawMenuBackground(self, withBanner=True):
        ...
        self.screen.blit(
            self.level.sprites.spriteCollection.get("player_idle").image,
            (2 * 32, 12 * 32),
        )
        self.screen.blit(
    ...
    def loadLevelNames(self):
        files = []
        res = []
        for r, d, f in os.walk("./assets/levels"):
            for file in f:
